...
- **Persistent disk cache**: `loadBlobFromCacheOrFetch()` uses the **Cache API** (`caches.open('musicality-audio-v1')`) so the audio bytes survive page reloads. On reload the page still has to rebuild the in-memory blob URLs, but it reads from disk instead of re-downloading from Bunny CDN — the prefetch progress bar fills almost instantly. The CDN is hit only on the first visit (or after the browser evicts the cache under storage pressure). Cache name is versioned so we can force a refill by bumping the suffix.
- **Offline mode**: a top-level toggle on the page (persisted to `localStorage` as `clipit-offline-mode`) controls whether the app talks to Bunny at all. Implemented via `store.isOfflineMode()` / `store.setOfflineMode()`.
  - **`init()`** in offline mode: skips the Bunny fetch entirely and reads metadata from a Cache API snapshot (`clipit-metadata-v1`) instead, falling back to bundled `default-metadata.json` if no snapshot exists yet.
  - **`syncToBunny`** in offline mode: writes the merged metadata to the local Cache API snapshot rather than PUTing to Bunny, and re-marks the dirty flags so the next online sync still pushes those edits to the server. Online edits also now snapshot to the cache as a side effect (via `cacheMetadataLocally` in `bunny.ts`).
  - **`ensureAudio()`** in offline mode: only reads from the audio Cache API; never falls through to `fetch()`. Songs that weren't downloaded ahead of time silently fail to play.
  - **Download-all button** on the page pre-fills the audio cache with every song that has musicality clips, then snapshots the current metadata. The page shows a live "X/N songs cached" count and (when supported) `navigator.storage.estimate().usage` in MB so you can see disk pressure.
  - **Clear cache button** appears once everything's cached, deletes both the audio cache (`musicality-audio-v1`) and the metadata snapshot (`clipit-metadata-v1`).
